    ...
    <>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        <div className="flex items-center justify-between">
          ...
          <Link href="/directory/submit" className="underline text-sm">
            Submit your service
          </Link>
        </div>
        ...
        <div className="flex gap-2 flex-wrap mt-4">
          <Link
            ...
            className={`text-xs px-2 py-1 rounded border ${
              !searchParams.category ? "bg-muted" : ""
            }`}
          >
            All
          </Link>
          {CATEGORIES.map((c) => (
            <Link
              ...
              className={`text-xs px-2 py-1 rounded border ${
                searchParams.category === c ? "bg-muted" : ""
              }`}
            >
              {c}
            </Link>
          ...
          {services.map((s: any) => (
            <div key={s._id} className="rounded-lg border p-4">
              <img
                src={
                  s.logo?.url ||
                  "/placeholder.svg?height=80&width=160&query=company+logo"
                }
